...
def Products_Details(request, id):
    ...
    related_products = Products.objects.filter(Category=pro.Category).exclude(id=pro.id)[:6]
    data = {
        "pro": pro,
...
def Products_Category_Show(request, category_id):
    ...
    current_category = Categoriys.objects.filter(id=category_id).first()
...
def Search_Products_results(request):
    ...
    if request.method == "GET":
        ...
        if query:
            ...
            results = Products.objects.filter(query_set).distinct()
        else:
